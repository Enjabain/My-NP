...
?>
</table></form><a style="float:right; margin-right:10px;" href="completedjobs.php">Completed Jobs</a></div>
<div class="rightmain">
      ...
      FROM member_types');
                            ...
                            <tr class="extra1"><th>Weekly/Monthly</th><td><label>Weekly:<input name="weeklymonthly" type="radio" value="weekly" checked /></label><label>Monthly:<input name="weeklymonthly" type="radio" value="monthly" id="showExtra2" /><label></td></tr>
                                            ...
                                            <div class="section"><div class="leftmain"><h2>Suggestions</h2>
                                                    <form id="editsuggestionform" name="editsuggestionform" method="post" action="editsuggestion.php">
                                                        <table><tr><th>Suggestion</th><th>Category</th><th>Suggested by</th><th>Votes</th><th>Status</th></tr>
                                                            <?php
                                                            $query5 = $db->prepare('
        SELECT *
        FROM mynp_suggestions LEFT JOIN members ON suggested_by = member_id');
                                                            ...
                                                            if ($query5->rowCount() != 0) {
                                                                foreach ($query5 as $row5) {
                                                                    $suggestion = $row5['suggestion'];
                                                                    $member_id = $row5['member_id'];
                                                                    $username = $row5['username'];
                                                                    $category = $row5['category'];
                                                                    $votes = $row5['votes'];
                                                                    $suggestion_id = $row5['suggestion_id'];
                                                                    $suggestion_status = $row5['suggestion_status'];
                                                                    ...
                                                                    $suggested_by = '<a href="member.php?member_id=' . $member_id . '">' . $username . '</a>';
                                                                    if ($member_id == '') {
                                                                        $suggested_by = 'Anonymous';
                                                                    }
                                                                    echo'<tr><td>' . $suggestion . '</td><td>' . $category . '</td><td>' . $suggested_by . '</td><td>' . $votes . '</td><td>' . $status . '<br /><button type="submit" value="' . $suggestion_id . '" name="suggestion_id">Edit</button></td></tr>';
                                                                }
                                                            }
                                                            ?>
                                                        </table></form></div></div>
