...
$statusupdate = htmlentities($_POST['statusupdate']);
...
//Create UPDATE query
if ($statusupdate != 'donothing') {
    if ($statusupdate == 'complete') {
        $job_status = 3;
    } elseif ($statusupdate == 'completionpending') {
        $job_status = 2;
    } elseif ($statusupdate == 'inprogress') {
        $job_status = 1;
        $seconds = 0;
    }
}
...
if ($statusupdate == 'forfeit') {
    $query2 = $db->prepare('UPDATE volunteer_opportunities
SET job_name = :job_name, job_type=:job_type, job_description = :job_description, job_bestfinishedby = :job_bestfinishedby, job_time = 0 , job_status = "0", job_inprogressby = NULL
WHERE job_id=:job_id');
    $result2 = $query2->execute(array('job_name' => $job_name, 'job_type' => $job_type, 'job_description' => $job_description, 'job_bestfinishedby' => $job_bestfinishedby, 'job_id' => $job_id));
} elseif ($statusupdate == 'delete') {
...
SET job_name = :job_name, job_type=:job_type, job_description = :job_description, job_bestfinishedby = :job_bestfinishedby, job_inprogressby =:job_inprogressby, job_time = :seconds, job_status = :job_status
WHERE job_id=:job_id');
    $result = $query->execute(array('job_name' => $job_name, 'job_type' => $job_type, 'job_description' => $job_description, 'job_bestfinishedby' => $job_bestfinishedby, 'job_inprogressby' => $job_inprogressby, 'seconds' => $seconds, 'job_status' => $job_status, 'job_id' => $job_id));
}
